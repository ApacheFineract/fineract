...
public class GuarantorsApiResource {
    ...
    @Operation(summary = "Retrieve Guarantor Account Linking Template",
            description = "Retrieves a JSON template with metadata required to link a guarantor’s savings account to a loan. "
    ...
    public GuarantorData newGuarantorTemplate(@PathParam("loanId") @NotNull(message = "{guarantor" +
            ".loanId.notNull}") @PositiveOrZero(message = "{guarantor.loanId.positiveOrZero}") @Digits(integer = 10, fraction = 0, message = "{guarantor.loanId.digits}") final Long loanId) {
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(summary = "Retrieve Guarantor Details",
            description = "Retrieves a guarantor details from the database. "
                    + "This API is part of The Apache Fineract platform's guarantor management module.")
    public List<GuarantorData> retrieveGuarantorDetails(@PathParam("loanId") @NotNull(message =
            "{guarantor.loanId.notNull}") @PositiveOrZero(message = "{guarantor.loanId.positiveOrZero}") @Digits(integer = 10, fraction = 0, message = "{guarantor.loanId.digits}") final Long loanId) {
        this.context.authenticatedUser().validateHasReadPermission(RESOURCE_NAME_FOR_PERMISSION);
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(summary = "Retrieve Guarantor Details with Optional Template Metadata",
            description = "Fetches details of a specific guarantor associated with a given loan using the `loanId` and `guarantorId` path parameters. "
                    ...
                    + "This API is part of The Apache Fineract platform's guarantor management module.")
    public GuarantorData retrieveGuarantorDetails(@Context final UriInfo uriInfo, @PathParam(
            "loanId") @NotNull(message = "{guarantor.loanId.notNull}") @PositiveOrZero(message = "{guarantor.loanId.positiveOrZero}") @Digits(integer = 10, fraction = 0, message = "{guarantor.loanId.digits}") final Long loanId,
    ...
    @Operation(summary = "Add a Guarantor to a Loan Account",
            description = "Creates and attaches a new guarantor to the specified loan account. "
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(summary = "Update an Existing Guarantor details on a Loan Account",
            description = "Updates the details of an existing guarantor associated with a specified loan account. "
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(summary = "Delete a Guarantor from a Loan Account",
            description = "Removes an existing guarantor associated with the specified loan account. "
                    ...
                    " stays in the database and it never deleted, only the relevant guarantor " +
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(summary = "Retrieve Guarantor Account Linking Options Template",
            description = "Provides a template containing available savings accounts for a client that can be linked as guarantor funding sources for a loan. "
    ...
    public GuarantorData accountsTemplate(
            @QueryParam("clientId") @NotNull(message = "{guarantor.clientId.notNull}") @PositiveOrZero(message = "{guarantor.clientId.positiveOrZero}") @Digits(integer = 10, fraction = 0, message = "{guarantor.clientId.digits}") final Long clientId,
            @PathParam("loanId") @NotNull(message = "{guarantor.loanId.notNull}") @PositiveOrZero(message = "{guarantor.loanId.positiveOrZero}") @Digits(integer = 10, fraction = 0, message = "{guarantor.loanId.digits}") final Long loanId) {
    ...
    @Operation(summary = "Retrieve Guarantor Account Linking Options Template",
            description = "Generates and downloads an Excel template for linking savings accounts as guarantor funding sources for a loan. "
    ...
    public Response getGuarantorTemplate(
            @QueryParam("officeId") @NotNull(message = "{guarantor.officeId.notNull}") @PositiveOrZero(message = "{guarantor.officeId.positiveOrZero}") @Digits(integer = 10, fraction = 0, message = "{guarantor.officeId.digits}") final Long officeId,
    ...
    @Operation(summary = "Upload Guarantor Account Linking Template",
            description = "Allows uploading of an Excel template used for linking guarantor savings accounts to a loan. "
                    ...
                    + "This API is part of The Apache Fineract platform's guarantor management module.")
    public String postGuarantorTemplate(
            @FormDataParam("file") InputStream uploadedInputStream,
            @FormDataParam("file") FormDataContentDisposition fileDetail,
            @FormDataParam("locale") @Locale final String locale,
