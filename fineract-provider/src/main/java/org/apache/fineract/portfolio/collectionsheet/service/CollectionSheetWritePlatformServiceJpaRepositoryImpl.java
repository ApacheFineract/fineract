...
public class CollectionSheetWritePlatformServiceJpaRepositoryImpl implements CollectionSheetWritePlatformService {
    ...
    @Override
    public CommandProcessingResult saveIndividualCollectionSheet(
        Command<SaveCollectionSheetRequest> command) {
...
//      }
      ...
      final JsonCommand jsonCommand = JsonCommand.from(json, parsedCommand, this.fromApiJsonHelper, wrapper.getEntityName(), wrapper.getEntityId(),
      ...
      return new CommandProcessingResultBuilder()
              .withCommandId((long)command.getId().hashCode())
              .with(changes).with(changes).build();
    }
